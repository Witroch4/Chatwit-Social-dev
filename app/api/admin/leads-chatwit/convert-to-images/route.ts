...
async function convertPdfToImagesWithImageMagick(pdfBuffer: Buffer, options: PDF2PicOptions): Promise<string[]> {
  try {
    ...
    try {
      const { stdout: pdfInfo } = await execPromise(`pdfinfo ${pdfPath} || echo "Não foi possível ler informações do PDF"`);
    ...
    if (files.length === 0) {
      ...
      try {
        const { stdout: policy } = await execPromise(`cat /etc/ImageMagick*/policy.xml || echo "Arquivo de política não encontrado"`);
  ...
  try {
    ...
    if (!pdfUrls || !Array.isArray(pdfUrls) || pdfUrls.length === 0) {
      ...
      if (!lead || !lead.pdfUnificado) {
        ...
        return NextResponse.json(
          { error: "Nenhum PDF unificado encontrado para este lead" },
          { status: 404 }
        );
      }
    ...
    for (const pdfUrl of pdfUrls) {
      try {
        ...
        try {
          ...
          log.info(`PDF baixado: ${pdfResponse.status} ${pdfResponse.statusText}, tamanho: ${pdfResponse.data.byteLength} bytes, tipo: ${pdfResponse.headers['content-type']}`);
          ...
          log.error(`Erro ao baixar PDF: ${downloadError instanceof Error ? downloadError.message : String(downloadError)}`);
