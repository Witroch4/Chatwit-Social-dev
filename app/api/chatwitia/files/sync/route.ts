...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ 
      error: 'Body inválido: {"fileId": "<string>"} ou {"storageUrl": "<string>", "filename": "<string>", "fileType": "<string>", "purpose": "<string>"}'
    }, { status: 400 });
  }
  ...
  if (size > 512 * 1024 * 1024)
    return NextResponse.json({ error: 'Arquivo excede 512 MB' }, { status: 413 });
