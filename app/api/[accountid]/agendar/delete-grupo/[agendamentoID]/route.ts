  ...
  try {
    ...
    try {
      ...
      const userAgendamentos = agendamentos.filter(
        (ag: any) => ag.userID === session.user.id
      );
      ...
      if (userAgendamentos.length === 0) {
        return NextResponse.json(
          { error: "Nenhum agendamento deste grupo pertence ao usuário." },
          { status: 403 }
        );
      }
      ...
      const deletePromises = userAgendamentos.map(async (ag: any) => {
        try {
          ...
          if (deleteError.response) {
            console.error(`[Agendar] Resposta de erro: ${deleteError.response.status} - ${JSON.stringify(deleteError.response.data)}`);
          }
      ...
      if (searchError.response) {
        console.error(`[Agendar] Resposta de erro: ${searchError.response.status} - ${JSON.stringify(searchError.response.data)}`);
      }
      return NextResponse.json(
        { error: "Erro ao buscar agendamentos", details: searchError.message },
        { status: 500 }
      );
    }
  } catch (error: any) {
