...
files.forEach(filePath => {
  try {
    ...
    const fixes = [
      ...
      { from: /toast\("([^"]+)",\s*\{\s*description:\s*"([^"]+)"\s*\}\s{2,}\);/g, to: 'toast("$1", { description: "$2" });' },
      { from: /toast\("([^"]+)",\s*\{\s*description:\s*([^}]+?)\s{2,}\}\s*\);/g, to: 'toast("$1", { description: $2 });' },
      
      // Corrigir toast.error e toast.success também
      { from: /toast\.error\("([^"]+)",\s*\{\s*description:\s*"([^"]+)"\s*\}\s{2,}\);/g, to: 'toast.error("$1", { description: "$2" });' },
      { from: /toast\.success\("([^"]+)",\s*\{\s*description:\s*"([^"]+)"\s*\}\s{2,}\);/g, to: 'toast.success("$1", { description: "$2" });' }
    ];
