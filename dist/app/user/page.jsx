...
const UserPage = () => {
    ...
    const [instagramData, setInstagramData] = (0, react_2.useState)(null);
    const [loading, setLoading] = (0, react_2.useState)(true);
    const [error, setError] = (0, react_2.useState)(null);
    (0, react_2.useEffect)(() => {
        const fetchInstagramData = async () => {
            var _a;
            if (status === "authenticated" && ((_a = session === null || session === void 0 ? void 0 : session.user) === null || _a === void 0 ? void 0 : _a.instagramAccessToken)) {
                try {
                    const response = await fetch(`https://graph.instagram.com/me?fields=id,username,media_count&access_token=${session.user.instagramAccessToken}`);
                    if (response.ok) {
    ...
    return (<div style={styles}>
      ...
      {loading ? (<p>Carregando dados do Instagram...</p>) : error ? (<p style={{ color: "red" }}>{error}</p>) : instagramData ? (<div>
        ...
        </div>) : (<p>Instagram não conectado ou dados não disponíveis.</p>)}
    </div>);
};
const styles = {
    padding: "2rem",
