...
exports.processLeadChatwitTask = processLeadChatwitTask;
 ...
 */
async function processLeadChatwitTask(job) {
 ...
 */
async function processAccumulatedJobs(sourceId) {
    if (!leadJobsCache[sourceId])
        return;
    ...
    try {
        ...
        let usuarioDb = await prisma_1.prisma.usuarioChatwit.findFirst({
            where: {
        ...
        if (usuarioDb) {
            usuarioDb = await prisma_1.prisma.usuarioChatwit.update({
            ...
            usuarioDb = await prisma_1.prisma.usuarioChatwit.create({
                data: {
                    userId: Number(usuario.account.id),
